...
impl Opts {
    ...
    pub(crate) fn to_js_value<'a, V, E>(&self, engine: &'a E) -> V
    where
        V: JsValue<'a>,
        E: JsEngine<JsValue<'a> = V>,
    {
        let mut opt: HashMap<String, V> = HashMap::new();
        if let Some(display_mode) = self.display_mode {
            opt.insert("displayMode".to_owned(), engine.from_bool(display_mode));
        }
        ...
        if let Some(leqno) = self.leqno {
            opt.insert("leqno".to_owned(), engine.from_bool(leqno));
        }
        if let Some(fleqn) = self.fleqn {
            opt.insert("fleqn".to_owned(), engine.from_bool(fleqn));
        }
        if let Some(throw_on_error) = self.throw_on_error {
            opt.insert("throwOnError".to_owned(), engine.from_bool(throw_on_error));
        }
        if let Some(error_color) = &self.error_color {
            opt.insert("errorColor".to_owned(), engine.from_string(error_color.clone()));
        }
        if !self.macros.is_empty() {
            opt.insert(
                ...
                engine.from_object(
                    self.macros
                        .iter()
                        .map(|(k, v)| (k.clone(), engine.from_string(v.clone()))),
                ),
            );
        }
        if let Some(min_rule_thickness) = self.min_rule_thickness {
            opt.insert(
                ...
                engine.from_float(min_rule_thickness),
            );
        }
        if let Some(Some(max_size)) = self.max_size {
            opt.insert("maxSize".to_owned(), engine.from_float(max_size));
        }
        if let Some(max_expand) = self.max_expand {
            match max_expand {
                Some(max_expand) => {
                    opt.insert("maxExpand".to_owned(), engine.from_int(max_expand));
                }
                None => {
                    opt.insert("maxExpand".to_owned(), engine.from_int(i32::max_value()));
                }
            }
        }
        if let Some(trust) = self.trust {
            opt.insert("trust".to_owned(), engine.from_bool(trust));
        }
        engine.from_object(opt.into_iter())
    }
}
